// ISO 3166 region → international dialling code. NANP members all share +1; visitors
// there type their area code as part of the number.
const DIAL_CODES: Record<string, string> = {
  ...
  KE: "254", KI: "686", XK: "383", KW: "965", KG: "996", LA: "856", LV: "371", LB: "961",
  ...
  NG: "234", KP: "850", MK: "389", NO: "47", OM: "968", PK: "92", PW: "680", PS: "970",
  PA: "507", PG: "675", PY: "595", PE: "51", PH: "63", PL: "48", PT: "351", PR: "1",
  QA: "974", RE: "262", RO: "40", RU: "7", RW: "250", KN: "1", LC: "1", VC: "1",
  WS: "685", SM: "378", ST: "239", SA: "966", SN: "221", RS: "381", SC: "248", SL: "232",
  SG: "65", SK: "421", SI: "386", SB: "677", SO: "252", ZA: "27", KR: "82", SS: "211",
  ES: "34", LK: "94", SD: "249", SR: "597", SE: "46", CH: "41", SY: "963", TW: "886",
  TJ: "992", TZ: "255", TH: "66", TL: "670", TG: "228", TO: "676", TT: "1", TN: "216",
  TR: "90", TM: "993", TV: "688", UG: "256", UA: "380", AE: "971", GB: "44", US: "1",
  UY: "598", UZ: "998", VU: "678", VA: "39", VE: "58", VN: "84", YE: "967", ZM: "260",
  ZW: "263",
};

// The site's audience is mostly South African.
export const DEFAULT_COUNTRY = "ZA";

// Italy, San Marino and the Vatican keep the leading 0 after the country code.
const KEEPS_TRUNK_ZERO = new Set(["IT", "SM", "VA"]);

const regionNames = new Intl.DisplayNames(["en"], { type: "region" });

export const COUNTRIES = Object.entries(DIAL_CODES)
  .map(([iso, dial]) => ({ iso, dial, name: regionNames.of(iso) ?? iso }))
  .sort((a, b) => a.name.localeCompare(b.name));

export function dialCode(iso: string) {
  return DIAL_CODES[iso] ?? DIAL_CODES[DEFAULT_COUNTRY];
}

/** Turns what the visitor typed into +<country><number>, or null when left blank.
 *  A number typed with its own + or 00 prefix wins over the dropdown. */
export function toInternational(raw: string, iso: string): string | null {
  const typed = raw.trim();
  if (!typed) return null;
  const digits = typed.replace(/\D/g, "");
  if (typed.startsWith("+")) return `+${digits}`;
  if (digits.startsWith("00")) return `+${digits.slice(2)}`;
  const national = KEEPS_TRUNK_ZERO.has(iso) ? digits : digits.replace(/^0/, "");
  return `+${dialCode(iso)}${national}`;
}

// Same rule as the enquiries.phone check constraint (E.164: 7–15 digits).
export const isValidPhone = (phone: string) => /^\+[1-9][0-9]{6,14}$/.test(phone);
