...
- **Supabase**: reused the existing project **OBXCreatives Project** (`lnyjveenyeqximpfxuia`, eu-central-1 / Frankfurt), restored from paused. Migration `create_enquiries` adds `public.enquiries` with length checks on every column, RLS on, and `anon` granted `insert (kind, name, email, message)` only. Migration `add_enquiry_phone_and_service` adds optional `phone` (E.164, e.g. `+27821234567`) and `service` (quotes only, free text so the list in `src/site.ts` can change without a migration); `anon` may insert those too. Security advisor: no findings.
...
- **Form**: `src/components/Enquiry.tsx`, section `#contact` (also answers to `#quote` and opens the quote tab). Honeypot field `website` absorbs simple bots. Phone has a country-code dropdown (default South Africa, logic in `src/lib/phone.ts`); the quote tab adds a required service dropdown (`SERVICES` in `src/site.ts`).
